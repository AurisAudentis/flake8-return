...
class ImplicitReturnMixin(Visitor):
    def _check_implicit_return(self, last_node: ast.AST) -> None:
        ...
        if isinstance(last_node, ast.Match):
            # We need at least one catch-all case, otherwise we'll consider the match to be non-exhaustive and therefore fail the linter
            if not any([isinstance(case, ast.MatchAs) and not case.pattern for case in last_node.cases]):
                self.error_from_node(ImplicitReturn, last_node)
